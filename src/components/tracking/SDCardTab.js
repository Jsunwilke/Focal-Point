...
const SDCardTab = ({ organizationID }) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setNotificationData({
          type: 'error',
          title: 'Error',
          message: 'Failed to load SD card records. Please try again.'
        });
      ...
      setNotificationData({
        type: 'error',
        title: 'Error',
        message: 'Unable to load SD card records. Please check your connection and try again.'
      });
      setShowNotificationModal(true);
    } finally {
      setLoading(false);
  ...
  const handleSearch = async () => {
    ...
    setLoading(true);
    try {
      const result = await searchSDCards(organizationID, sanitizedSearchTerm, searchField);
      if (result.success) {
        const recordsWithNames = await resolveUserNames(result.data, organizationID);
        setRecords(recordsWithNames);
      } else {
        setNotificationData({
          type: 'error',
          title: 'Search Failed',
          message: 'Unable to search SD cards. Please try again.'
        });
        setShowNotificationModal(true);
      }
    } catch (error) {
      setNotificationData({
        type: 'error',
        title: 'Search Error',
        message: 'Unable to search SD cards. Please check your connection and try again.'
      });
      setShowNotificationModal(true);
    } finally {
      setLoading(false);
  ...
  const handleDelete = (recordId) => {
    setConfirmModalData({
      title: 'Delete Record',
      message: 'Are you sure you want to delete this SD card record? This action cannot be undone.',
      confirmText: 'Delete',
      cancelText: 'Cancel',
      type: 'danger',
    ...
    try {
      ...
      if (result.success) {
        ...
        setNotificationData({
          type: 'success',
          title: 'Success',
          message: 'SD card record deleted successfully.',
          autoClose: true
        });
        setShowNotificationModal(true);
      } else {
